...
var import_memoize2 = __toESM(require("lodash/memoize.js"), 1);
...
var import_deburr = __toESM(require("lodash/deburr.js"), 1);
var import_isPlainObject2 = __toESM(require("lodash/isPlainObject.js"), 1);
var import_trim = __toESM(require("lodash/trim.js"), 1);
var import_upperFirst = __toESM(require("lodash/upperFirst.js"), 1);

// src/transforms/zui-to-typescript/types/JSONSchema.ts
var import_memoize = __toESM(require("lodash/memoize.js"), 1);
...
function toSafeString(string) {
  return (0, import_upperFirst.default)(
    // remove accents, umlauts, ... by their basic latin letters
    (0, import_deburr.default)(string).replace(/(^\s*[^a-zA-Z_$])|([^a-zA-Z_$\d])/g, " ").replace(/^_[a-z]/g, (match) => match.toUpperCase()).replace(/_[a-z]/g, (match) => match.substr(1, match.length).toUpperCase()).replace(/([\d$]+[a-zA-Z])/g, (match) => match.toUpperCase()).replace(/\s+([a-zA-Z])/g, (match) => (0, import_trim.default)(match.toUpperCase())).replace(/\s/g, "")
  );
}
...
function maybeStripDefault(schema) {
  ...
  switch (schema.type) {
    ...
    case "null":
      ...
      break;
    case "object":
      if ((0, import_isPlainObject2.default)(schema.default)) {
        return schema;
      }
      break;
  }
...
function isSchemaLike(schema) {
  if (!(0, import_isPlainObject2.default)(schema)) {
    return false;
  }
  const parent = schema[Parent];
  if (parent === null) {
...
var generateType = (0, import_memoize2.default)(generateTypeUnmemoized);
...
function generateStandaloneType(ast, options) {
  return (hasComment(ast) ? generateComment(ast.comment) + "\n" : "") + `export type ${toSafeString(ast.standaloneName)} = ${generateType(
    (0, import_omit.default)(ast, "standaloneName"),
    options
  )}`;
}
...
var import_isPlainObject3 = __toESM(require("lodash/isPlainObject.js"), 1);
function link(schema, parent = null) {
  if (!Array.isArray(schema) && !(0, import_isPlainObject3.default)(schema)) {
    return schema;
  }
...
var import_findKey = __toESM(require("lodash/findKey.js"), 1);
var import_includes = __toESM(require("lodash/includes.js"), 1);
var import_isPlainObject5 = __toESM(require("lodash/isPlainObject.js"), 1);
var import_map = __toESM(require("lodash/map.js"), 1);
var import_memoize3 = __toESM(require("lodash/memoize.js"), 1);
var import_omit2 = __toESM(require("lodash/omit.js"), 1);

// src/transforms/zui-to-typescript/typesOfSchema.ts
var import_isPlainObject4 = __toESM(require("lodash/isPlainObject.js"), 1);
...
var matchers = {
  ...
  OBJECT(schema) {
    return schema.type === "object" && !(0, import_isPlainObject4.default)(schema.additionalProperties) && !schema.allOf && !schema.anyOf && !schema.oneOf && !schema.patternProperties && !schema.properties && !schema.required;
  },
  ONE_OF(schema) {
...
function parseNonLiteral(schema, type, options, keyName, processed, usedNames) {
  ...
  const keyNameFromDefinition = (0, import_findKey.default)(definitions, (_) => _ === schema);
  switch (type) {
    case "ALL_OF":
      return {
    ...
    case "UNION":
      return {
        ...
        params: schema.type.map((type2) => {
          const member = __spreadProps(__spreadValues({}, (0, import_omit2.default)(schema, "$id", "description", "title")), { type: type2 });
...
function parseSchema(schema, options, processed, usedNames, parentSchemaName) {
  let asts = (0, import_map.default)(schema.properties, (value, key) => ({
    ast: parse(value, options, key, processed, usedNames),
    isPatternProperty: false,
    isRequired: (0, import_includes.default)(schema.required || [], key),
  ...
  if (schema.patternProperties) {
    ...
    asts = asts.concat(
      (0, import_map.default)(schema.patternProperties, (value, key) => {
...
${comment}` : comment;
        return {
          ...
          isRequired: singlePatternProperty || (0, import_includes.default)(schema.required || [], key),
          isUnreachableDefinition: false,
  ...
  if (options.unreachableDefinitions) {
    asts = asts.concat(
      (0, import_map.default)(schema.$defs, (value, key) => {
        const ast = parse(value, options, key, processed, usedNames);
...
function getDefinitions(schema, isSchema = true, processed = /* @__PURE__ */ new Set()) {
  ...
  if ((0, import_isPlainObject5.default)(schema)) {
...
var getDefinitionsMemoized = (0, import_memoize3.default)(getDefinitions);
...
async function compile(schema, name = "Root", options = {}) {
  ...
  const _options = (0, import_merge.default)({}, DEFAULT_OPTIONS, options);
  const start = Date.now();
  function time() {
    return `(${Date.now() - start}ms)`;
  }
  __name(time, "time");
  if (!(0, import_endsWith.default)(_options.cwd, "/")) {
    _options.cwd += "/";
  }
  const _schema = (0, import_cloneDeep.default)(schema);
  const { dereferencedPaths, dereferencedSchema } = await dereference(_schema, _options);
  if (process.env.VERBOSE) {
    if ((0, import_isEqual2.default)(_schema, dereferencedSchema)) {
...
var _ZodNumber = class _ZodNumber extends ZodType {
  ...
  _parse(input4) {
    ...
    for (const check of this._def.checks) {
      if (check.kind === "int") {
        if (!util2.isInteger(input4.data)) {
          ctx = this._getOrReturnCtx(input4, ctx);
          addIssueToContext(ctx, {
            code: ZodIssueCode.invalid_type,
            expected: "integer",
...
var _ZodTemplateLiteral = class _ZodTemplateLiteral extends ZodType {
  ...
  _transformPartToRegexString(part) {
    ...
    if (part instanceof ZodEnum || part instanceof ZodNativeEnum) {
      const values = part instanceof ZodEnum ? part._def.values : util2.getValidEnumValues(part._def.values);
...
var mergeZuiIntoJsonSchema = /* @__PURE__ */ __name((jsonSchema, zuiSchema, opts) => {
  ...
  if (jsonSchema.type == "array") {
    ...
    if (Array.isArray(jsonSchema.items)) {
      ...
      if (def.typeName === z_exports.ZodFirstPartyTypeKind.ZodMap) {
...
var parseAnyOf = /* @__PURE__ */ __name((schema, refs) => {
  return schema.anyOf.length ? schema.anyOf.length === 1 ? parseSchema2(schema.anyOf[0], __spreadProps(__spreadValues({}, refs), {
    path: [...refs.path, "anyOf", 0]
  })) : `z.union([${schema.anyOf.map((schema2, i) => parseSchema2(schema2, __spreadProps(__spreadValues({}, refs), { path: [...refs.path, "anyOf", i] }))).join(", ")}])` : `z.any()`;
}, "parseAnyOf");
...
var parseMultipleType = /* @__PURE__ */ __name((schema, refs) => {
  return `z.union([${schema.type.map((type) => parseSchema2(__spreadProps(__spreadValues({}, schema), { type }), refs)).join(", ")}])`;
}, "parseMultipleType");

// src/transforms/json-schema-to-zui/parsers/parseNot.ts
var parseNot = /* @__PURE__ */ __name((schema, refs) => {
  return `z.any().refine((value) => !${parseSchema2(schema.not, __spreadProps(__spreadValues({}, refs), {
    path: [...refs.path, "not"]
...
var omit3 = /* @__PURE__ */ __name((obj, ...keys) => Object.keys(obj).reduce((acc, key) => {
...
function parseAllOf(schema, refs) {
  if (schema.allOf.length === 0) {
    ...
    return parseSchema2(item, __spreadProps(__spreadValues({}, refs), {
...
var parseArray = /* @__PURE__ */ __name((schema, refs) => {
  if (Array.isArray(schema.items)) {
    return `z.tuple([${schema.items.map((v, i) => parseSchema2(v, __spreadProps(__spreadValues({}, refs), { path: [...refs.path, "items", i] })))}])`;
  }
  let r = !schema.items ? "z.array(z.any())" : `z.array(${parseSchema2(schema.items, __spreadProps(__spreadValues({}, refs), {
...
var parseIfThenElse = /* @__PURE__ */ __name((schema, refs) => {
  const $if = parseSchema2(schema.if, __spreadProps(__spreadValues({}, refs), { path: [...refs.path, "if"] }));
  const $then = parseSchema2(schema.then, __spreadProps(__spreadValues({}, refs), {
    path: [...refs.path, "then"]
  }));
  const $else = parseSchema2(schema.else, __spreadProps(__spreadValues({}, refs), {
    path: [...refs.path, "else"]
  }));
...
var parseOneOf = /* @__PURE__ */ __name((schema, refs) => {
  return schema.oneOf.length ? schema.oneOf.length === 1 ? parseSchema2(schema.oneOf[0], __spreadProps(__spreadValues({}, refs), {
    path: [...refs.path, "oneOf", 0]
  })) : `z.any().superRefine((x, ctx) => {
    const schemas = [${schema.oneOf.map(
    (schema2, i) => parseSchema2(schema2, __spreadProps(__spreadValues({}, refs), {
...
function parseObject(objectSchema, refs) {
  ...
  if (objectSchema.properties) {
    if (!Object.keys(objectSchema.properties).length) {
      ...
      properties += Object.keys(objectSchema.properties).map((key) => {
        ...
        const result = `${JSON.stringify(key)}: ${parseSchema2(propSchema, __spreadProps(__spreadValues({}, refs), {
  ...
  const additionalProperties = objectSchema.additionalProperties !== void 0 ? parseSchema2(objectSchema.additionalProperties, __spreadProps(__spreadValues({}, refs), {
  ...
  if (objectSchema.patternProperties) {
    const parsedPatternProperties = Object.fromEntries(
      Object.entries(objectSchema.patternProperties).map(([key, value]) => {
        return [
          key,
          parseSchema2(value, __spreadProps(__spreadValues({}, refs), {
...
var parseNullable = /* @__PURE__ */ __name((schema, refs) => {
  return `${parseSchema2(omit3(schema, "nullable"), refs, true)}.nullable()`;
}, "parseNullable");
...
var parseSchema2 = /* @__PURE__ */ __name((schema, refs = { seen: /* @__PURE__ */ new Map(), path: [] }, blockMeta) => {
...
var useFormData = /* @__PURE__ */ __name((fieldSchema, path) => {
  ...
  const data = (0, import_react.useMemo)(() => getPathData(context.formData, path), [context.formData, path]);
  ...
  const hiddenMask = (0, import_react.useMemo)(() => parseMaskableField("hidden", fieldSchema, data), [fieldSchema, data]);
  const disabledMask = (0, import_react.useMemo)(() => parseMaskableField("disabled", fieldSchema, data), [fieldSchema, data]);
  (0, import_react.useEffect)(() => {
  ...
  const { disabled, hidden } = (0, import_react.useMemo)(() => {
    const hidden2 = hiddenMask === true || getPathData(context.hiddenState, path);
    const disabled2 = disabledMask === true || getPathData(context.disabledState, path);
  ...
  const handlePropertyChange = (0, import_react.useCallback)(
    (path2, data2) => {
      context.setFormData(setObjectPath(context.formData, path2, data2));
    },
    [context.formData]
  );
  const addArrayItem = (0, import_react.useCallback)(
    (path2, data2) => {
      const currentData = getPathData(context.formData, path2) || [];
      context.setFormData(setObjectPath(context.formData, path2, [...currentData, data2]));
    },
    [context.formData]
  );
  const removeArrayItem = (0, import_react.useCallback)(
    (path2, index) => {
      ...
      context.setFormData(setObjectPath(context.formData, path2, currentData));
    },
    [context.formData]
  );
  return __spreadValues(__spreadProps(__spreadValues({}, context), { data, disabled, hidden, handlePropertyChange, addArrayItem, removeArrayItem }), validation);
}, "useFormData");
...
var FormElementRenderer = /* @__PURE__ */ __name((_a) => {
  ...
  const data = (0, import_react5.useMemo)(() => getPathData(formData, path), [formData, path]);
  const componentMeta = (0, import_react5.useMemo)(() => resolveComponent(components, fieldSchema), [fieldSchema, components]);
  ...
  if (fieldSchema.type === "array" && type === "array") {
    ...
    const props2 = __spreadValues(__spreadProps(__spreadValues({}, baseProps), {
      ...
      schema,
      data: dataArray,
      addItem: (data2 = void 0) => addArrayItem(path, typeof data2 === "undefined" ? getDefaultItemData(schema.items) : data2),
      removeItem: (index) => removeArrayItem(path, index)
    }), childProps);
    if (Array.isArray(fieldSchema.items)) {
      return null;
    }
    return /* @__PURE__ */ import_react5.default.createElement(Component3, __spreadProps(__spreadValues({ key: baseProps.scope }, props2), { isArrayChild: props2.isArrayChild }), Array.isArray(props2.data) ? props2.data.map((_, index) => {
  ...
  if (fieldSchema.type === "object" && type === "object" && fieldSchema.properties) {
    ...
    return /* @__PURE__ */ import_react5.default.createElement(Component3, __spreadProps(__spreadValues({ key: baseProps.scope }, props2), { isArrayChild: props2.isArrayChild }), Object.entries(fieldSchema.properties).map(([fieldName, childSchema]) => {
      var _a3;
      const childPath = [...path, fieldName];
      return /* @__PURE__ */ import_react5.default.createElement(ErrorBoundary, { key: childPath.join("."), fallback, fieldSchema: childSchema, path: childPath }, /* @__PURE__ */ import_react5.default.createElement(
        FormElementRenderer,
        {
          key: childPath.join("."),
          components,
  ...
  if (type === "discriminatedUnion") {
    ...
    return /* @__PURE__ */ import_react5.default.createElement(Component3, __spreadProps(__spreadValues({ key: baseProps.scope }, props2), { isArrayChild: props2.isArrayChild }), discriminatedSchema && /* @__PURE__ */ import_react5.default.createElement(ErrorBoundary, { key: path.join("."), fallback, fieldSchema: discriminatedSchema, path }, /* @__PURE__ */ import_react5.default.createElement(
      FormElementRenderer,
      {
        components,
  ...
  return /* @__PURE__ */ import_react5.default.createElement(Component2, __spreadValues({}, props));
}, "FormElementRenderer");
